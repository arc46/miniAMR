...
//         timer_comm_send[dir] += t4 - t3;
      ...
      c1 = c2 = c3 = 0;
//loop-level removed - #pragma omp parallel for private (n, bp, l, m, i, j, k, t2, time1, time2, \
   ...
   */
   if (!code) {
      if ((l/2) == 0) {         /* West, East */
         ...
         #pragma omp task depend(inout: barray[start*block3D_size:num_comm*block3D_size], \
   ...
   */
   if (!code) {
      ...
      if ((l/2) == 0) {
         ...
         #pragma omp task depend(inout: barray[start*block3D_size:num_comm*block3D_size], \
                                        barray1[start*block3D_size:num_comm*block3D_size]) \
                          firstprivate(start, num_comm, barray, barray1, iq, jq, \
   ...
   if (!code && stencil == 7)
      switch (l) {
         ...
         case 0: for (var = start; var < start+num_comm; var++) {
                    block3D_t array = (block3D_t)&barray[var*block3D_size];
                    for (j = 1; j <= y_block_size; j++)
